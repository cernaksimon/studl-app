...
function Employers(props) {
    ...
    const contains = ({ title, description, email }, query) => {
        if (title.includes(query) || description.includes(query) || email.includes(query)) {
            return true;
        }
        ...
        return (
            <View style={styles.card}>
                <View style={styles.row}>
                    ...
                    <Text style={styles.rating}>{rating.average_score?rating.average_score+"/5":rating.message}</Text> 
                </View>
        ...
        <View style={{ flex: 1, padding: 24 }}>
            {isLoading ? <ActivityIndicator /> : (
                <FlatList
                    ...
                    data={data}
                    renderItem={({ item }) => (
                        <Job title={item.title} description={item.description} email={item.email} phone={item.phone} rating={item.rating} />
                    )}
                />
            )}
        </View>
